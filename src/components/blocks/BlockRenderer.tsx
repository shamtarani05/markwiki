...
export function BlockRenderer({
  ...
  switch (block.type) {
    case 'heading': {
      ...
      return (
        <Tag id={block.id} className="scroll-mt-24 border-b border-outline-variant/30 pb-2">
          {block.props.text}
        </Tag>
      );
    }
    ...
    case 'image':
      return (
        <figure>
          ...
          {block.props.caption && (
            <figcaption className="text-xs text-on-surface-variant text-center mt-2">
              {block.props.caption}
            </figcaption>
          )}
        </figure>
    ...
    case 'videoEmbed':
      return block.props.url ? (
        <figure>
          <div className="aspect-video rounded-lg overflow-hidden border border-outline-variant/30">
    ...
    case 'quote':
      return block.props.text ? (
        <blockquote className="border-l-4 border-accent bg-surface-container-low p-4 rounded-r-lg not-italic">
          <p className="text-lg italic text-on-surface m-0">&ldquo;{block.props.text}&rdquo;</p>
          {block.props.source && (
            <footer className="text-sm text-on-surface-variant mt-2">— {block.props.source}</footer>
          )}
        </blockquote>
      ) : null;
      ...
      return (
        <aside className="card overflow-hidden w-full sm:max-w-sm">
          <div className="bg-primary/20 p-3 text-center">
            <h3 className="font-bold text-on-surface text-lg m-0">{block.props.title || 'Untitled'}</h3>
            {block.props.subtitle && (
              <p className="text-xs text-on-surface-variant m-0">{block.props.subtitle}</p>
            )}
          </div>
          {block.props.image && (
            <div>
              ...
              {block.props.imageCaption && (
                <p className="text-xs text-center text-on-surface-variant p-2 bg-surface-container-low m-0">
                  {block.props.imageCaption}
                </p>
              )}
            </div>
            ...
            <dl className="divide-y divide-border m-0">
              {block.props.fields.map((f, i) => (
                <div key={i} className="p-3">
                  ...
                  <dd className="text-sm text-on-surface m-0">{f.value || '—'}</dd>
                </div>
              ))}
            </dl>
          )}
    ...
    case 'comments':
      return (
        <div className="card p-4">
          ...
          <p className="text-sm text-on-surface-variant">Comments open once this page is published.</p>
        </div>
      );
    ...
    case 'blogPosts':
      return (
        <div className="ad-zone">
          <span>{BLOCK_LABELS[block.type]} — live on the published page</span>
        </div>
      );
...
function CardGridItem({ item }: { item: { title: string; subtitle?: string; image?: string; href?: string } }) {
  const content = (
    <div className="card overflow-hidden h-full">
      ...
      <div className="p-3">
        <h4 className="font-semibold text-on-surface text-sm m-0">{item.title}</h4>
        {item.subtitle && <p className="text-xs text-on-surface-variant mt-1 m-0">{item.subtitle}</p>}
      </div>
    </div>
  );
...
function TableOfContents({ title, headings }: { title?: string; headings: Extract<Block, { type: 'heading' }>[] }) {
  ...
  return (
    <div className="card p-4 bg-surface-container-low not-prose">
      <div className="flex items-center justify-between cursor-pointer" onClick={() => setOpen(!open)}>
        <h2 className="font-semibold text-on-surface m-0">{title || 'Contents'}</h2>
        <button className="text-primary text-sm hover:underline">[{open ? 'hide' : 'show'}]</button>
      </div>
      {open && (
        <nav className="mt-3">
          <ol className="space-y-1 text-sm list-none pl-0">
            {headings.map((h, i) => (
              <li key={h.id} className={h.props.level === 3 ? 'ml-6' : ''}>
                <button onClick={() => scrollTo(h.id)} className="text-primary hover:underline flex items-start gap-2">
...
function NewsletterBlock({ title, subtitle }: { title?: string; subtitle?: string }) {
  ...
  return (
    <div className="card p-6 bg-surface-container-low text-center">
      ...
      {submitted ? (
        ...
        >
          <input
            ...
            className="flex-1 px-4 py-2 bg-surface-container-lowest border border-outline-variant/30 rounded-lg text-on-surface placeholder:text-on-surface-variant focus:outline-none focus:border-accent transition-colors"
          />

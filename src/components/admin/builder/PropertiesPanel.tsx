...
export default function PropertiesPanel({
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-semibold text-[#706F78] uppercase tracking-wide pr-6">
        {block.type} settings
      </h3>
  ...
  return (
    <>
      ...
      <div className="space-y-2">
        <span className="text-xs text-[#706F78]">Images</span>
        {props.images.map((img, i) => (
          <div key={i} className="border border-[rgba(255,255,255,0.09)] rounded-lg p-2 space-y-1">
...
function FieldListEditor({ label, items, onChange }: { label: string; items: InfoboxField[]; onChange: (items: InfoboxField[]) => void }) {
  ...
  return (
    <div className="space-y-2">
      <span className="text-xs text-[#706F78]">{label}</span>
      {items.map((f, i) => (
        <div key={i} className="border border-[rgba(255,255,255,0.09)] rounded-lg p-2 space-y-1">
...
function CardItemListEditor({ items, onChange }: { items: CardGridItem[]; onChange: (items: CardGridItem[]) => void }) {
  ...
  return (
    <div className="space-y-2">
      <span className="text-xs text-[#706F78]">Items</span>
      {items.map((it, i) => (
        <div key={i} className="border border-[rgba(255,255,255,0.09)] rounded-lg p-2 space-y-1">
...
function TextField({ label, value, onChange, compact }: { label: string; value: string; onChange: (v: string) => void; compact?: boolean }) {
  return (
    <label className="block">
      <span className="block text-xs text-[#706F78] mb-1">{label}</span>
      <input
        type="text"
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className={`w-full ${compact ? 'px-2 py-1 text-xs' : 'px-3 py-2 text-sm'} bg-[#0B0B0F] border border-[rgba(255,255,255,0.09)] rounded-lg text-[#F5F3EF] focus:outline-none focus:border-accent transition-colors`}
      />
    </label>
  );
}
...
function AddButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="text-xs text-[#8B5CF6] hover:underline">
      + {label}
    </button>
  );
}
